...
IF OBJECT_ID ( '[EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo]', 'P' ) IS NOT NULL 
    DROP PROCEDURE EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo;
GO

CREATE PROCEDURE EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo
	  @sToken as varchar(24)
	, @iIdConsolidado as int
	, @sPeriodo as varchar(6)
	, @sPeriodoO as varchar(6)
...
BEGIN
	SET NOCOUNT ON;
	--
	Set @sName = 'EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo';
	Set @sDeepP = @sDeep +'__';
	Set @sDeep  = @sDeep + '__';
	--
	...
	Set @sTexto = @sTexto + ' PeriodoO {' + @sPeriodoO + '}';
	Set @sTexto = @sTexto + ' Libros {' + @sLibros + '}' ;
	Execute EERR_sp_Log4Sql_info @sName, @sTexto;

	Begin Try
		Set @sTexto = @sDeep + ' Proceso de Generacion ';
		Execute EERR_sp_Log4Sql_Info @sName, @sTexto;
		...
		Set @sTexto = @sDeep + '__ Transformacion de Periodos en tabla para consolidados a comparar';
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
		--
		Set @sPer = @sPeriodo + ',' + @sPeriodoO;
		WITH Separa(pn, start, stop) AS (
		...
		--
			Set @curAnidados = CURSOR LOCAL STATIC READ_ONLY FORWARD_ONLY FOR 
				Select CodigoReferenciado, Codigo, TipoNodo, idRegistro From EERR_Tbl_Consolidados
				 Where idPadre = @iIdConsolidado order by TipoNodo;
			Open @curAnidados;
			Fetch Next From @curAnidados Into @iIdAnidado, @sCodigo, @iTipoNodo, @iIdRegistroOrg
			While @@FETCH_STATUS = 0
				Begin
					Set @sTexto = @sDeep + '___ Registro sCodigo {' + @sCodigo + '} TipoNodo {' + Convert( varchar(1), @iTipoNodo) + '}';
					Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
					...
					If @iTipoNodo = 1
						Begin
							...
							Execute @iRetVal = EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo @sToken, @iIdAnidado, @sPeriodo, @sPeriodoO, @sLibros, @sDeepP;
							If @iRetVal = 1 
								Begin
									Set @sTexto = @sDeep + '__ Devolvio Error';
									Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
									Return @iRetVal;
								End;
							Set @sTexto = @sDeep + '___ Inserta Registros de ajustes del consolidado ' + convert(varchar(10), @iIdAnidado);
							Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
							Insert into EERR_Tmp_Token_ESF
								Select
									  @sToken
									, @iIdConsolidado
					...
					Else
						Begin
							Set @sTexto = @sDeep + '___ Inserta Registros de saldos de la empresa';
							Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
							Insert into EERR_Tmp_Token_ESF
								Select
									  @sToken
