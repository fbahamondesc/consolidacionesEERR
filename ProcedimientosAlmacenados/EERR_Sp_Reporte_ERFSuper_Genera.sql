...
IF OBJECT_ID ( '[EERR_Sp_Reporte_ERFSuper_Genera]', 'P' ) IS NOT NULL 
    DROP PROCEDURE EERR_Sp_Reporte_ERFSuper_Genera;
GO

CREATE PROCEDURE EERR_Sp_Reporte_ERFSuper_Genera
...
Declare @sIdGrupoX as varchar(6);
...
Declare @TablaTemporal Table ( 
								...
								, idGrupo varchar(6)					-- Este codigo incluye el tipo para uso de orden
...
Declare @TablaTemporalPaso Table ( 
								idConsolidado int
								, DescripcionConsolidado varchar(500)
								, idCompania varchar(18)
								, DescripcionCompania varchar(500)
...
BEGIN
	SET NOCOUNT ON;
	--
	Set @sName = 'EERR_Sp_Reporte_ERFSuper_Genera'; 
	...
	Begin Try
		Set @sTexto = 'Inicio Proceso de Generacion de Reporte ERFSuper';
		...
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			...
			Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			EXEC @iRetVal = EERR_Sp_Reporte_ERFSuper_ExtraeSaldos @iIdConsolidado, @sPeriodo, @sLibros, '______', @CursorSaldos = @SaldosCursor OUTPUT;
			If @iRetVal = 1 
				Begin
			...
			Set @sTexto = '____ Llama a EERR_Sp_Reporte_ERF_ExtraeSaldos';
			Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			EXEC @iRetVal = EERR_Sp_Reporte_ERFSuper_ExtraeSaldos @iIdConsolidadoComparar, @sPeriodoComparar, @sLibros, '______', @CursorSaldos = @SaldosCursor OUTPUT;
			If @iRetVal = 1 
				Begin
					Set @sTexto = '____ Devolvio Error';
					Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
		...
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			...
			While @@FETCH_STATUS = 0
				Begin
					...
					Execute @iRetVal = EERR_Sp_Reporte_ERFSuper_Recursivo_ExtraeSaldosyAjustes @sToken, @iIdAnidado, @sPeriodo, @sLibros, '______';
					If @iRetVal = 1 
						Begin
							Set @sTexto = '____ Devolvio Error';
							Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			...
			While @@FETCH_STATUS = 0
				Begin
					...
					set @iIdAnidado = Case when @iIdAnidado = 0 then @iIdAnidadoOrg Else @iIdAnidado End;
					-- Llamado al recursivo
					Execute @iRetVal = EERR_Sp_Reporte_ERFSuper_Recursivo_ExtraeSaldosyAjustes_Comparativo @sToken, @iIdAnidado, @sPeriodoComparar, @sPeriodo, @sLibros, '______';
					If @iRetVal = 1 
						Begin
							Set @sTexto = '____ Devolvio Error';
							Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
							Return @iRetVal;
						End;
					-- Insertamos todas los Sadlos que extraje del consolidado
					Set @sTexto = '_____ Insertamos los saldos que extrajo';
					Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
					Insert Into @TablaTemporal
						Select  
				...
				Begin
					...
					Insert Into @TablaTemporal
						Select 						
							...
							, (Select Distinct DescripcionConsolidado From @TablaTemporalPaso  Where idConsolidado = @iIdConsolidadoRellena ) Cdes
							, @sIdCompaniaRellena
							, (Select Distinct DescripcionCompania From @TablaTemporalPaso  Where idConsolidado = @iIdConsolidadoRellena And IdCompania = @sIdCompaniaRellena ) DCOmpania
							, MGR.Tipo +  MGR.Codigo
							...
							, CGCC.IdCuenta
							, MCU.Descripcion
							, (Select Distinct IdPeriodo From @TablaTemporalPaso  Where idConsolidado = @iIdConsolidadoRellena ) IPeriodo
							, 0 Valor
							, MCU.FlagImprime
							, 'RE'						
						From
		...
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			...
			Set @curSumatoria = Cursor LOCAL STATIC READ_ONLY FORWARD_ONLY  For
				...
				Order By idConsolidado, idPeriodo, idCompania, idGrupo, idconcepto
			Open @curSumatoria;
			...
			Insert into @TablaTemporal
					Select 
						...
						, Case When A.idConsolidado > 0 Then @sCodigoConsolidado + '-' + @sDescripcionConsolidado
								Else @sCodigoConsolidadoComparar + '-' + @sDescripcionConsolidadoComparar End 
							,A.idCompania 
						, Case When Len(A.idCompania) = 1 Then 'Ajustes'
								When A.idConsolidado > 0 Then 
									( Select C.DescripcionCompania From  @TablaTemporal C Where 1 = 1
										And  C.idConsolidado  = A.idConsolidado And C.idCompania = A.idCompania
										Group By C.DescripcionCompania ) 
		...
		--
		--
		-- Set @sTexto = '____ Transformacion de los datos en dos columnas';
		-- Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
		----
		--select * from @TablaTemporal 
		...
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			...
			Fetch Next From @curRegistros Into 
					...
					, @sTipoLinea
					, @sIdCuentaX
			Set @sTexto = '____ Transformacion de los datos en dos columnas - Antes del ciclo';
			Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
			While @@FETCH_STATUS = 0
				Begin
					Set @sTexto = '____ Transformacion de los datos en dos columnas - en ciclo - grupo:' + @sIdGrupoX +  ' - ' + @sDescripcionGrupoX + ' - Cuenta:' + @sIdCuentaX;
					Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
					
					Set @iSoloAjuste = 
									( Select FlagSoloAjuste From EERR_Tbl_Maestro_Cuentas  Where IdCuenta = @sIdCuentaX );
					If @iFlagImprimeX = 1 OR ( @iFlagImprimeX = 0 And @nValorX <> 0  And @iSoloAjuste = 0 And @sIdCuentaX not like '%ERF%')
						Begin
					...
					Fetch Next From @curRegistros Into 
							...
							, @sIdCuentaX
				End;		
		Set @sTexto = '____ Fin Transformacion de los datos en dos columnas';
		Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
		--
