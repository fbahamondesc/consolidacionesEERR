...
IF OBJECT_ID ( '[EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes]', 'P' ) IS NOT NULL 
    DROP PROCEDURE EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes;
GO

CREATE PROCEDURE EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes
...
BEGIN
	SET NOCOUNT ON;
	--
	Set @sName = 'EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes';
	Set @sDeepP = @sDeep +'__';
	--
	...
	Begin Try
		...
		--
			...
			While @@FETCH_STATUS = 0
				Begin
					Set @sTexto = @sDeep + '____ Registro sCodigo {' + @sCodigo + '} TipoNodo {' + Convert( varchar(1), @iTipoNodo) + '}';
					Execute EERR_sp_Log4Sql_Debug @sName, @sTexto;
					...
					If @iTipoNodo = 1
						Begin
							...
							Execute @iRetVal = EERR_Sp_Reporte_ESFSuper_Recursivo_ExtraeSaldosyAjustes @sToken, @iIdAnidado, @sPeriodo, @sLibros, @sDeepP;
							If @iRetVal = 1 
								Begin
